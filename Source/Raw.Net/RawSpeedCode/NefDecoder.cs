...
bool NefDecoder::D100IsCompressed(UInt32 offset) {
  byte *test = mFile.getData(offset, 256);
  int i;
...
void NefDecoder::DecodeUncompressed() {
  ...
  for (UInt32 s = 0; s < nslices; s++) {
    ...
    offY = Math.Math.Min(((height, offY + yPerSlice);
...
void NefDecoder::readCoolpixMangledRaw(ByteStream &input, iPoint2D& size, iPoint2D& offset, int inputPitch) {
  ...
  UInt32 y = offset.y;
  h = Math.Math.Min(((h + (UInt32)offset.y, (UInt32)mRaw.dim.y);
  w *= cpp;
...
void NefDecoder::readCoolpixSplitRaw(ByteStream &input, iPoint2D& size, iPoint2D& offset, int inputPitch) {
  ...
  BitPumpMSB in(&input);
  for (; y < h; y++) {
    UInt16* dest = (UInt16*) & data[offset.x*sizeof(UInt16)*cpp+y*2*outPitch];
    for (UInt32 x = 0 ; x < w; x++) {
      dest[x] =  in.getBits(12);
    }
  }
  for (y = offset.y; y < h; y++) {
    UInt16* dest = (UInt16*) & data[offset.x*sizeof(UInt16)*cpp+(y*2+1)*outPitch];
    for (UInt32 x = 0 ; x < w; x++) {
      dest[x] =  in.getBits(12);
    }
  }
...
void NefDecoder::DecodeNikonSNef(ByteStream &input, UInt32 w, UInt32 h) {
  ...
  for (UInt32 y = 0; y < h; y++) {
    ...
    for (UInt32 x = 0 ; x < w*3; x += 6) {
      ...
      mRaw.setWithLookUp(clampbits((int)(y1 - 0.337633 * cb - 0.698001 * cr), 12), (byte8*)&dest[x+1], &random);
      ...
      mRaw.setWithLookUp(clampbits((int)(y2 - 0.337633 * cb2 - 0.698001 * cr2), 12), (byte8*)&dest[x+4], &random);
...
#define SQR(x) ((x)*(x))
UInt16* NefDecoder::gammaCurve(double pwr, double ts, int mode, int imax) {
  UInt16 *curve = (UInt16*)_aligned_malloc(65536 * sizeof(UInt16), 16);
  if (curve == null) {
