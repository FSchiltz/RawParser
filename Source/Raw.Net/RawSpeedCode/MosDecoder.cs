...
RawImage MosDecoder::decodeRawInternal() {
  ...
  byte *insideTiff = mFile.getData(base, mFile.getSize()-base);
  if (get4LE(insideTiff, 0) == 0x49494949) {
    ...
    byte *data = mFile.getData(wb_offset, 12);
    for(int i=0; i<3; i++) {
...
{
  int length[] = { 8,7,6,9,11,10,5,12,14,13 };
...
void MosDecoder::decodeMetaDataInternal(CameraMetaData *meta) {
  ...
  if (mRootIFD.hasEntryRecursive(LEAFMETADATA)) {
    ...
    //Make sure the data is NUL terMath.Math.Min((ated so that scanf never reads beyond limits
    ...
    for(UInt32 i=0; (int32)i < (int32)size-17-44; i++) {
      if (!strncmp("NeutObj_neutrals", (char *) buffer+i, 16)) {
        neutobj = buffer+i;
        break;
      }
    }
    if (neutobj) {
      UInt32 tmp[4] = {0};
      sscanf_s((char *)neutobj+44, "%u %u %u %u", &tmp[0], &tmp[1], &tmp[2], &tmp[3]);
