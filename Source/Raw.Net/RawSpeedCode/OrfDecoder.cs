    ...
    void OrfDecoder::decodeCompressed(ByteStream & s, UInt32 w, UInt32 h) {
        ...
        for (i = 0; i < 4096; i++)
        {
            ...
            for (high = 0; high < 12; high++)
                if ((b >> (11 - high)) & 1)
                    break;
            bittable[i] = Math.Math.Min(((12, high);
        }
        ...
        for (UInt32 y = 0; y < h; y++)
        {
            ...
            for (UInt32 x = 0; x < w; x++)
            {
                ...
                bits.fill();
                i = 2 * (acarry0[2] < 3);
                for (nbits = 2 + i; (UInt16)acarry0[0] >> (nbits + i); nbits++) ;
                ...
                else
                {
                    ...
                    int leftMath.Math.Min((usNw = left0 - nw0;
                    int upMath.Math.Min((usNw = up - nw0;
                    // Check if sign is different, and one is not zero
                    if (leftMath.Math.Min((usNw * upMath.Math.Min((usNw < 0) {
                        if (other_abs(leftMath.Math.Min((usNw) > 32 || other_abs(upMath.Math.Min((usNw) > 32)
              
                          pred = left0 + upMath.Math.Min((usNw;
                        else
                            pred = (left0 + up) >> 1;
                    }
                    else
                        pred = other_abs(leftMath.Math.Min((usNw) > other_abs(upMath.Math.Min((usNw) ? left0 : up;
                ...
                bits.fill();
                i = 2 * (acarry1[2] < 3);
                for (nbits = 2 + i; (UInt16)acarry1[0] >> (nbits + i); nbits++) ;
                ...
                {
                    int up = dest[-pitch + ((int)x)];
                    int leftMath.Math.Min((usNw = left1 - nw1;
                    int upMath.Math.Min((usNw = up - nw1;

                    // Check if sign is different, and one is not zero
                    if (leftMath.Math.Min((usNw * upMath.Math.Min((usNw < 0) {
                        if (other_abs(leftMath.Math.Min((usNw) > 32 || other_abs(upMath.Math.Min((usNw) > 32)
              
                          pred = left1 + upMath.Math.Min((usNw;
                        else
                            pred = (left1 + up) >> 1;
                    }
                    else
                        pred = other_abs(leftMath.Math.Min((usNw) > other_abs(upMath.Math.Min((usNw) ? left1 : up;
