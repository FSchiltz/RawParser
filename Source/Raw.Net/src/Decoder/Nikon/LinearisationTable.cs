...
namespace RawNet
{
    public class LinearisationTable : IDisposable
    {
         ...
         * 
         */
        public LinearisationTable(ushort compressionType, int colordepth, uint Rawoffset, uint Linetableoffset, TIFFBinaryReader r,TIFFBinaryReader imageReader)
        {
            //rawdataLength = table.Length;
            this.colordepth = colordepth;

            //move the reader to the corresponding offset
            r.Position = Linetableoffset;
            ...
            */
            ...
            if (version0 == 0x44 && version1 == 0x20 && step > 0)
            {
                ...
                r.Position = Linetableoffset + 562;
            ...
            imageReader.Position = Rawoffset;
            byte[] imageBuffer = new byte[imageReader.BaseStream.Length - imageReader.Position];
            imageReader.BaseStream.Read(imageBuffer, 0, imageBuffer.Length);

            rawdataLength = imageBuffer.Length;
            reader = new TIFFBinaryReader(new MemoryStream(imageBuffer));
            reader.Position = 0;
        }
        ...
        /*
          ...
          * 
          */
        public ushort[] uncompressed(uint height, uint width)
        {
            ...
            for (min = row = 0; row < height; row++)
            {
                ...
                for (col = 0; col < width; col++)
                {
                    ...
                    uncompressedData[(((int)(row * width) + col) * 3)] = curve[lim((short)hpred[col & 1], 0, 0x3fff)];
        ...
        public uint getbithuff(int nbits, ushort[] huff)
        {
            ...
            while (reset == 0 && vbits < nbits && reader.Position < reader.BaseStream.Length)// && !(c == 0xff && reader.ReadByte()== 0))
            {
                c = reader.ReadByte();
