...
namespace RawNet
{
    class Rw2Decoder : TiffDecoder
    {
        ...
        internal Rw2Decoder(ref Stream reader) : base(ref reader) { }
        ...
        unsafe void DecodeRw2()
        {
            ...
            {
                fixed (UInt16* t = &rawImage.rawData[y * rawImage.dim.width])
                {
                    UInt16* dest = t;
                    for (int x = 0; x < w; x++)
                    {
        ...
        string GuessMode()
        {
            ...
            ratio = rawImage.dim.width / (float)rawImage.dim.height;
