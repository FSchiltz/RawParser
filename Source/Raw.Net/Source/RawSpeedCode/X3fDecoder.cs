...
protected:
  ...
  string getId();
  ByteStream *bytes;
  bool hasProp(stringkey) { return mProperties.props.find(key) != mProperties.props.end();};
  string getProp(stringkey);
...
void X3fDecoder::checkSupportInternal( CameraMetaData *meta )
{
  ...
  ThrowRDE("X3F Decoder: Unable to deterMath.Math.Min((e camera name.");
}

string X3fDecoder::getProp(stringkey )
{
...
void X3fDecoder::decompressSigma( X3fImage &image )
{
  ...
  if (image.format == 30 || image.format == 35) {
    ...
    if (image.format == 35) {
      ...
      for (int i = 0; i < 2;  i++) {
        for (int y = 0; y < h; y++) {
          UInt16* dst = (UInt16*)mRaw.getData(0, y * 2 )+ i;
          UInt16* dst_down = (UInt16*)mRaw.getData(0, y * 2 + 1) + i;
          UInt16* blue = (UInt16*)mRaw.getData(0, y * 2) + 2;
          UInt16* blue_down = (UInt16*)mRaw.getData(0, y * 2 + 1) + 2;
          for (int x = 0; x < w; x++) {
  ...
  if (image.format == 6) {
    ...
    for (int i = 0; i < 1024; i++) {
      ...
      max_len = Math.Max(max_len, val>>27);
    }
    if (max_len>26)
    ...
    huge_table = (UInt16*)_aligned_malloc((1<<max_len)*2, 16);
    if (!huge_table)
...
void X3fDecoder::decodeThreaded( RawDecoderThread* t )
{
  if (curr_image.format == 30 || curr_image.format == 35) {
    ...
    for (int y = 0; y < dim.y; y++) {
      UInt16* dst = (UInt16*)mRaw.getData(0, y << subs) + i;
  ...
  if (curr_image.format == 6) {
    ...
    for (UInt32 y = t.start_y; y < t.end_y; y++) {
      ...
      UInt16* dst = (UInt16*)mRaw.getData(0,y);
      pred[0] = pred[1] = pred[2] = 0;
      for (int x = 0; x < mRaw.dim.x; x++) {
        for (int i = 0; i < 3; i++) {
          UInt16 val = huge_table[bits.peekBits(max_len)];
          byte nbits = val&31;
          if (val == 0xffff) {
