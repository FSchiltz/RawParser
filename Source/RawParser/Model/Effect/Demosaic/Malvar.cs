        ...
        public static void Demosaic(RawImage image)
        {
            ...
            for (long y = 0; y < image.raw.dim.height; y++)
            {
                for (long x = 0; x < image.raw.dim.width; x++, i++)
                {
                    ...
                    {
                        /* Center pixel is red */
                        image.raw.green[i] = (ushort)((2 * (Neigh[2, 1] + Neigh[1, 2]
                            + Neigh[3, 2] + Neigh[2, 3])
                        ...
                        image.raw.blue[i] = (ushort)((4 * (Neigh[1, 1] + Neigh[3, 1]
                            + Neigh[1, 3] + Neigh[3, 3]) +
                    ...
                    else if ((x & 1) == BlueX && (y & 1) == BlueY)
                    {
                        ...
                        image.raw.green[i] = (ushort)((2 * (Neigh[2, 1] + Neigh[1, 2]
                            ...
                            / (2 * (NeighPresence[2, 1] + NeighPresence[1, 2]
                            + NeighPresence[3, 2] + NeighPresence[2, 3])));
                        image.raw.red[i] = (ushort)((4 * (Neigh[1, 1] + Neigh[3, 1]
                            + Neigh[1, 3] + Neigh[3, 3]) +
                            3 * ((NeighPresence[0, 2] + NeighPresence[4, 2]
                            + NeighPresence[2, 0] + NeighPresence[2, 4]) * Neigh[2, 2]
                            - Neigh[0, 2] - Neigh[4, 2]
                    ...
                    else
                    {
                        ...
                        if ((y & 1) == redy)
                        {
                            ...
                            image.raw.red[i] = (ushort)((8 * (Neigh[1, 2] + Neigh[3, 2])
                            ...
                            image.raw.blue[i] = (ushort)((8 * (Neigh[2, 1] + Neigh[2, 3])
                                + (2 * (NeighPresence[1, 1] + NeighPresence[3, 1]
                        ...
                        else
                        {
                            ...
                            image.raw.red[i] = (ushort)((8 * (Neigh[2, 1] + Neigh[2, 3])
                                + (2 * (NeighPresence[1, 1] + NeighPresence[3, 1]
                                + NeighPresence[2, 0] + NeighPresence[2, 4]
                                + NeighPresence[1, 3] + NeighPresence[3, 3])
                                - NeighPresence[0, 2] - NeighPresence[4, 2]) * Neigh[2, 2]
                                ...
                                + Neigh[0, 2] + Neigh[4, 2])
                                / (8 * (NeighPresence[2, 1] + NeighPresence[2, 3])));
                            image.raw.blue[i] = (ushort)((8 * (Neigh[1, 2] + Neigh[3, 2])
                                + (2 * (NeighPresence[1, 1] + NeighPresence[3, 1]
                                + NeighPresence[0, 2] + NeighPresence[4, 2]
                                + NeighPresence[1, 3] + NeighPresence[3, 3])
                                - NeighPresence[2, 0] - NeighPresence[2, 4]) * Neigh[2, 2]
